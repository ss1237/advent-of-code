...
public class AOC2021D04P2 {
	...
	public static void main(String[] args) throws IOException {
		...
		while (in.hasNextLine()) {
			...
			int newWinTurn = nums.size();
			
			for (int i = 0; i < 5; i++) {
				if (rows[i][1] == 5) {
					newWinTurn = Math.min(newWinTurn, rows[i][0]);
				}
				if (cols[i][1] == 5) {
					newWinTurn = Math.min(newWinTurn, cols[i][0]);
				}
			}
			
			//out.println(Arrays.deepToString(rows));
			//out.println(Arrays.deepToString(cols));
			//out.println(newWinTurn);
			
			if (newWinTurn > maxTurn) {
				int unmarked = 0;
				for (int i = 0; i < 5; i++) {
					for (int j = 0; j < 5; j++) {
						if (nums.indexOf(board[i][j]) == -1 || nums.indexOf(board[i][j]) > newWinTurn) {
							unmarked += board[i][j];
						}
					}
				}
				
				val = unmarked * nums.get(newWinTurn);
